...
void checkBgTasks(){
    //handleServer();
    handleSerialListen();
    handleAutonomousSwich();
    handleClient();
}

void handleAutonomousSwich(){
    if(!is_autonomous)
        return;

    if(currentLight == 1 && espMillis() - autoTime > redTime){
        autoTime     = espMillis();
        currentLight = 2;
    ...
    if(currentLight == 2 && espMillis() - autoTime > greenTime){
        autoTime     = espMillis();
        currentLight = 3;
        digitalWrite(RELAY_PIN, HIGH);
        setBuzzer(BUZZER_GOOD_SOUND, 1000);
    ...
    if(currentLight == 3 && espMillis() - autoTime > yellowTime){
        autoTime     = espMillis();
        currentLight = 1;
