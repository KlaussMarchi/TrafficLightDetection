...
void routesSetup(){
    ...
    server.on("/HIGH",   HTTP_GET,  onRelayHighRequest);
    server.on("/LOW",    HTTP_GET,  onRelayLowRequest);
    server.on("/RESET",  HTTP_GET,  onResetRequest);
    server.on("/AUTO",   HTTP_POST, onAutoRequest);
}
...
void wifiSetup(){
    String esp_id = settings["esp_id"].as<String>();
